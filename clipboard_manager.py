...
def on_press(key):
    try:
        ...
        elif key == keyboard.Key.shift: current_keys.add('shift')
...
def on_release(key):
    try:
        ...
        elif key == keyboard.Key.shift: current_keys.discard('shift')
...
print(f"Persistent Clipboard Manager started at {START_TIME}")
try:
    root.mainloop()
